...
def test_is_quiet_time_simple_range():
    ...
    # 15:00 is inside
    dt_inside = datetime(2024, 1, 1, 15, 0, tzinfo=timezone.utc)
    assert is_quiet_time(dt_inside, q_start, q_end, tz) is True
    
    # 13:00 is outside
    dt_outside = datetime(2024, 1, 1, 13, 0, tzinfo=timezone.utc)
...
def test_is_quiet_time_crossing_midnight():
    ...
    # 01:00 is inside
    dt_inside = datetime(2024, 1, 1, 1, 0, tzinfo=timezone.utc)
    assert is_quiet_time(dt_inside, q_start, q_end, tz) is True
    
    # 23:30 is inside
    dt_inside_late = datetime(2024, 1, 1, 23, 30, tzinfo=timezone.utc)
    assert is_quiet_time(dt_inside_late, q_start, q_end, tz) is True
    
    # 12:00 is outside
    dt_outside = datetime(2024, 1, 1, 12, 0, tzinfo=timezone.utc)
    assert is_quiet_time(dt_outside, q_start, q_end, tz) is False
...
def test_is_quiet_time_different_timezone():
    ...
    dt_utc = datetime(2024, 1, 1, 15, 0, tzinfo=timezone.utc)
    ...
    dt_utc_day = datetime(2024, 1, 1, 3, 0, tzinfo=timezone.utc)
