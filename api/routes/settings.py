...
def setup_routes_settings(app: web.Application) -> None:
    ...
    async def update_settings(request: web.Request) -> web.Response:
        ...
        if "daily_limit" in body:
            try:
                ...
                return web.json_response({"ok": False, "error": "invalid_number"}, status=400)
        ...
        if "practice_mode" in body:
            ...
            if mode not in {"word_to_translation", "translation_to_word"}:
        ...
        if "quiet_start" in body or "quiet_end" in body:
            ...
            if (quiet_start is not None and not _is_valid_time(quiet_start)) or (
                quiet_end is not None and not _is_valid_time(quiet_end)
            ):
                return web.json_response({"ok": False, "error": "invalid_time_format"}, status=400)
    ...
    async def get_api_token(request: web.Request) -> web.Response:
        ...
        if not token:
            ...
            logger.info("User %d generated initial API token", request["telegram_id"])
